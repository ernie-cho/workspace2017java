...
public class UDPServer {
	...
	public static void main(String[] args) {
		final int port = 9988;	// 포트 번호를 final 변수로 선언
		...
		try{
			...
			while(true){
				try{
					...
					else if(R_Str2.contains("안녕")){
						Out_bf = "반가워 :)".getBytes();
					}
					else if(R_Str2.contains("이름")){
						Out_bf = "저는 SERO예요. 그쪽 이름은요?".getBytes();
					}
					else if(R_Str2.contains("몰라")){
						Out_bf = "저랑 대화하기 싫어요?".getBytes();
					}
					else if(R_Str2.contains("응")){
						Out_bf = "네~".getBytes();
					}
					else if(R_Str2.contains("영")){
						Out_bf = "그렇군요".getBytes();
					}
					else if(R_Str2.contains("남자")){
						Out_bf = "전 성별이 없어요 ~".getBytes();
					}
					else if(R_Str2.contains("공부")){
						Out_bf = "쉬운 공부가 어딨겠어요~".getBytes();
					}
					else if(R_Str2.contains("분식")){
						Out_bf = "떡튀순이 최고지!".getBytes();
					}
					else if(R_Str2.contains("음료")){
						Out_bf = "콜라나 먹어".getBytes();
					}
					else if(R_Str2.contains("햄버거")){
